...
OUTPUT_ROOT = "/content/processed_pika_v2"
METADATA_CSV = "/content/processed_pika_v2/pika_context_metadata.csv"
...
def load_first_label_from_pill_json(label_path):
    ...
    first_ann = anns[0]
    if "label" not in first_ann:
        return None

    return int(first_ann["label"])
...
def main():
    ...
    # Build quick lookup: pill json -> main label
    pill_main_label = {}
    ...
    for label_file in tqdm(label_files):
        ...
        label_value = load_first_label_from_pill_json(label_path)
        if label_value is not None:
            pill_main_label[label_file] = label_value
    ...
    print("Building context metadata...")
    for label_file in tqdm(label_files):
        label_path = os.path.join(pill_label_dir, label_file)
        ...
        all_related_pills = pres_to_pills.get(pres_file, [])
        context_labels = []

        for related_pill_json in all_related_pills:
            if related_pill_json == label_file:
                continue
            if related_pill_json in pill_main_label:
                context_labels.append(pill_main_label[related_pill_json])

        # Remove duplicates but keep sorted for readability
        context_labels = sorted(list(set(context_labels)))
