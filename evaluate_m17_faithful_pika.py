...
def load_graph_embeddings(checkpoint, graph_artifacts_dir):
    graph_path = checkpoint.get("graph_embeddings_path", "")

    candidate_paths = []

    if graph_path:
        candidate_paths.append(Path(graph_path))
    ...
    raise FileNotFoundError(
        "Cannot find graph_embeddings.npy. Please provide --graph_artifacts_dir."
    )


def build_model_from_checkpoint(checkpoint, graph_embeddings, device):
    num_classes = int(checkpoint["num_classes"])
    pill_model_name = checkpoint.get("pill_model_name", "tf_efficientnetv2_s.in21k_ft_in1k")
    pres_model_name = checkpoint.get("pres_model_name", "resnet18.a1_in1k")
    hidden_dim = int(checkpoint.get("hidden_dim", 256))
    graph_dim = int(checkpoint.get("graph_dim", graph_embeddings.shape[1]))

    print("Model type        :", checkpoint.get("model_type", "M17FaithfulPIKAModel"))
...
def evaluate(model, loader, device, num_classes):
    ...
    for pill_imgs, pres_imgs, context_indices, context_mask, labels in tqdm(loader, desc="Evaluating M17"):
...
def main(args):
    ...
    print("=== EVALUATE M17 FAITHFUL PIKA ===")
    ...
    with open(output_dir / "m17_test_metrics.json", "w", encoding="utf-8") as f:
    ...
    pred_df.to_csv(output_dir / "m17_test_predictions.csv", index=False)
    ...
    pd.DataFrame(report_dict).T.to_csv(output_dir / "m17_classification_report.csv")
    ...
    per_class_df.to_csv(output_dir / "m17_per_class_metrics.csv", index=False)

    cm = confusion_matrix(y_true, y_pred, labels=labels_all)
    pd.DataFrame(cm, index=target_names, columns=target_names).to_csv(
        output_dir / "m17_confusion_matrix.csv"
    )

    print("\nSaved files:")
    print(output_dir / "m17_test_metrics.json")
    print(output_dir / "m17_test_predictions.csv")
    print(output_dir / "m17_classification_report.csv")
    print(output_dir / "m17_per_class_metrics.csv")
    print(output_dir / "m17_confusion_matrix.csv")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate M17 faithful PIKA checkpoint on clean test split."
    )
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/content/drive/MyDrive/model/M17_faithful_pika/test_eval",
    )
