...
def main(args):
    ...
    print("=== EVALUATE M21 STRONG VISUAL PIKA ===")
    ...
    with open(output_dir / "m21_test_metrics.json", "w", encoding="utf-8") as f:
    ...
    pred_df.to_csv(output_dir / "m21_test_predictions.csv", index=False)
    ...
    pd.DataFrame(report_dict).T.to_csv(output_dir / "m21_classification_report.csv")
    ...
    per_class_df.to_csv(output_dir / "m21_per_class_metrics.csv", index=False)

    cm = confusion_matrix(y_true, y_pred, labels=labels_all)
    pd.DataFrame(cm, index=target_names, columns=target_names).to_csv(
        output_dir / "m21_confusion_matrix.csv"
    )

    print("\nSaved files:")
    print(output_dir / "m21_test_metrics.json")
    print(output_dir / "m21_test_predictions.csv")
    print(output_dir / "m21_classification_report.csv")
    print(output_dir / "m21_per_class_metrics.csv")
    print(output_dir / "m21_confusion_matrix.csv")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate M21 strong visual PIKA checkpoint on clean test split."
    )
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/content/drive/MyDrive/model/M21_strong_visual_pika/test_eval",
    )
