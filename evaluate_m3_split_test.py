...
from train_pika_v2_context_labels import (
    ContextPIKAModel,
    ContextPIKADataset,
    add_mapped_columns,
    check_image_paths,
...
@torch.no_grad()
def predict_m3(model, loader, device, idx_to_label):
    model.eval()
...
def main(args):
    ...
    print("Model type        :", ckpt.get("model_type", "ContextPIKAModel"))
    ...
    test_dataset = ContextPIKADataset(
        test_df,
    ...
    pred_df, y_true, y_pred = predict_m3(
        model=model,
        loader=test_loader,
        device=device,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate M3 ContextPIKAModel checkpoint on split test set")
