...
from train_pika_v3_triple_context import (
    TripleContextPIKA,
    PIKAV3Dataset,
    add_mapped_columns,
    check_image_paths,
...
@torch.no_grad()
def predict_m4(model, loader, device, idx_to_label):
    model.eval()
    ...
    for pill_imgs, pres_imgs, context_vectors, labels in loader:
...
def main(args):
    ...
    context_dim = int(ckpt.get("context_dim", args.context_dim))

    print("Model type        :", ckpt.get("model_type", "TripleContextPIKA"))
    ...
    pred_df, y_true, y_pred = predict_m4(
        model=model,
        loader=test_loader,
        device=device,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate M4 TripleContextPIKA checkpoint on split test set")
