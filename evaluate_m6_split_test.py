...
@torch.no_grad()
def predict_m6(model, loader, device, idx_to_label):
    model.eval()
...
def main(args):
    ...
    pred_df, y_true, y_pred = predict_m6(
        model=model,
        loader=test_loader,
        device=device,
        idx_to_label=idx_to_label,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate M6 Best PIKA checkpoint on split test set")
