...
HIDDEN_DIM = 256
MAX_CONTEXT_LEN = 8
OUTPUT_DIR = "./outputs_best_pika_finetune_v4"

EARLY_STOPPING_PATIENCE = 4
GRAD_CLIP_NORM = 1.0

LR_PILL_ENCODER = 3e-6
LR_PRES_ENCODER = 3e-6
LR_GRAPH_ENCODER = 7e-6
LR_FUSION_HEAD = 3e-5
LR_CLASSIFIER = 4e-5

FOCAL_GAMMA = 1.5
CB_BETA = 0.999
HARD_CLASS_BOOST = 1.35

BEST_MODEL_PATH = os.path.join(OUTPUT_DIR, "best_pika_model_finetuned_v4.pth")
RESUME_CHECKPOINT_PATH = os.path.join(OUTPUT_DIR, "resume_checkpoint_v4.pth")

# hard classes observed from v3 report
HARD_CLASSES_ORIGINAL = {15, 37, 85, 88, 101}
...
def main():
    ...
    # map hard classes from original label ids to mapped ids
    hard_classes_mapped = set()
    for original_label in HARD_CLASSES_ORIGINAL:
        if original_label in old_to_new:
            hard_classes_mapped.add(old_to_new[original_label])
    ...
    print("Hard classes mapped:", sorted(list(hard_classes_mapped)))
    ...
    class_weights = effective_num_weights(counts_arr, beta=CB_BETA)

    # boost hard classes slightly
    for cls in hard_classes_mapped:
        class_weights[cls] *= HARD_CLASS_BOOST
    ...
    for y in train_df["mapped_label"].tolist():
        base = 1.0 / math.sqrt(max(train_counts[y], 1))
        if y in hard_classes_mapped:
            base *= HARD_CLASS_BOOST
    ...
    criterion = ClassBalancedFocalLoss(class_weights=class_weights, gamma=FOCAL_GAMMA)
