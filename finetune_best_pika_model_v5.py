...
BASE_CHECKPOINT = os.environ.get(
    ...
    "/content/vaipe-contextual-pill-recognition/outputs_best_pika_finetune_v3/best_pika_model_finetuned_v3.pth"
)
...
HIDDEN_DIM = 256
MAX_CONTEXT_LEN = 8
OUTPUT_DIR = "./outputs_best_pika_finetune_v5"
...
# Smaller and softer learning rates than v4
LR_PILL_ENCODER = 1e-6
LR_PRES_ENCODER = 1e-6
LR_GRAPH_ENCODER = 2e-6
LR_FUSION_HEAD = 2e-5
LR_CLASSIFIER = 3e-5

EMA_DECAY = 0.999

BEST_MODEL_PATH = os.path.join(OUTPUT_DIR, "best_pika_model_finetuned_v5.pth")
RESUME_CHECKPOINT_PATH = os.path.join(OUTPUT_DIR, "resume_checkpoint_v5.pth")
...
class EMA:
    ...
    def load_state_dict(self, shadow_state):
...
def set_train_mode(model, freeze_backbones=True):
    # always train these
    for p in model.pill_proj.parameters():
        p.requires_grad = True
    for p in model.pres_proj.parameters():
        p.requires_grad = True
...
@torch.no_grad()
def validate_one_epoch(model, loader, criterion, ema, device):
...
def save_resume_checkpoint(model, optimizer, scheduler, scaler, ema, epoch, best_f1, bad_epochs, freeze_backbones, path):
    torch.save({
        "epoch": epoch,
...
def main():
    ...
    # clip weights nhẹ để tránh quá mạnh tay
    class_weights = np.clip(class_weights, 0.80, 1.25)
    ...
    if os.path.exists(RESUME_CHECKPOINT_PATH):
        ...
        freeze_backbones = resume_ckpt.get("freeze_backbones", True)
    ...
    for epoch in range(start_epoch, EPOCHS):
        # freeze phase
        freeze_backbones = epoch < FREEZE_EPOCHS
        set_train_mode(model, freeze_backbones=freeze_backbones)
        ...
        train_loss, train_acc, train_f1 = train_one_epoch(
            model, train_loader, criterion, optimizer, scaler, ema, device
        )
        val_loss, val_acc, val_f1, val_labels, val_preds = validate_one_epoch(
            model, val_loader, criterion, ema, device
        )
        scheduler.step()
        ...
        if val_f1 > best_f1:
            ...
            torch.save(ema.state_dict(), BEST_MODEL_PATH)
            print(f"Saved best EMA model with Val Macro F1 = {best_f1:.4f}")
            bad_epochs = 0
        else:
            bad_epochs += 1
