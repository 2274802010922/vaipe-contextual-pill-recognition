...
def build_weighted_sampler(train_df, exponent=0.5, max_weight_ratio=20.0):
    """
    Balanced sampler for long-tail classes.

    exponent:
        0.0 = no balancing
        0.5 = inverse sqrt frequency
        1.0 = inverse frequency, very strong

    max_weight_ratio:
        cap sample weights to avoid over-repeating classes with 1-2 samples.
    """
...
def train_one_epoch(model, loader, criterion, optimizer, scaler, device):
    ...
    pbar = tqdm(loader, desc="M12 Balanced Focal Training", leave=False)
...
def validate_one_epoch(model, loader, criterion, device):
    ...
    pbar = tqdm(loader, desc="M12 Validation", leave=False)
...
def main(args):
    ...
    print("Class weight exponent:", args.class_weight_exponent)
    print("Sampler exponent:", args.sampler_exponent)
    print("Focal gamma:", args.focal_gamma)
    print("Label smoothing:", args.label_smoothing)
    print("Dropout override:", args.dropout_p)
    ...
    # Important:
    # New stratified val split is different from the checkpoint's old validation split.
    # Therefore, do not initialize best_f1 from base checkpoint.
    best_f1 = -1.0
    bad_epochs = 0
    history = []
    ...
    print("Starting best_f1 for new stratified validation:", best_f1)

    for epoch in range(1, args.epochs + 1):
        print(f"\nM12 Stratified Balanced Focal Epoch [{epoch}/{args.epochs}]")
        ...
        checkpoint = {
            ...
            "stage": "M12_stratified_balanced_focal",
            "loss_type": args.loss_type,
            "focal_gamma": args.focal_gamma,
            "class_weight_exponent": args.class_weight_exponent,
            "sampler_exponent": args.sampler_exponent,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="M12 stratified balanced focal fine-tune for Best PIKA model."
    )
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/content/drive/MyDrive/model/M12_stratified_balanced_focal",
    )

    parser.add_argument(
        "--best_name",
        type=str,
        default="M12_stratified_balanced_focal_best.pth",
    )

    parser.add_argument(
        "--last_name",
        type=str,
        default="M12_stratified_balanced_focal_last.pth",
    )
