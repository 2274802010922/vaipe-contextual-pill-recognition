...
def parse_hard_negative_labels(x):
    if pd.isna(x):
        return []
...
class M24HardMetricDataset(Dataset):
    def __init__(self, df, transform=None, hard_weight_max=1.8):
...
class ClassBalancedBatchSampler(Sampler):
    """
    Tạo batch có nhiều class và mỗi class có >=2 samples để SupCon có positive pairs.
    Có sampling theo class weight nhẹ, ưu tiên hard class nhưng không quá mạnh.
    """
...
class CosFaceHead(nn.Module):
    def __init__(self, embedding_dim, num_classes, scale=30.0, margin=0.15):
        super().__init__()
...
class M24MetricVisualModel(nn.Module):
    def __init__(
        self,
        num_classes,
    ...
    def forward(self, images, labels=None):
        ...
        logits = self.head(emb, labels=labels)
...
class WeightedCELoss(nn.Module):
    def __init__(self, class_weights=None, label_smoothing=0.03):
        super().__init__()
...
class HardPairNegativeLoss(nn.Module):
    def __init__(self, hard_pair_matrix, temperature=0.10, margin=0.10):
        super().__init__()

        self.register_buffer("hard_pair_matrix", hard_pair_matrix.float())
        self.temperature = float(temperature)
        self.margin = float(margin)
    ...
    def forward(self, embeddings, labels):
        ...
        label_i = labels.view(-1, 1)
        label_j = labels.view(1, -1)

        hard_mask = self.hard_pair_matrix[label_i, label_j].squeeze(1)

        eye = torch.eye(labels.shape[0], device=labels.device).bool()
        hard_mask = hard_mask.bool() & (~eye)

        if hard_mask.sum() == 0:
            return embeddings.new_tensor(0.0)

        hard_sim = sim[hard_mask]

        # Penalize hard negative pairs that are too similar.
        loss = F.softplus((hard_sim - self.margin) / self.temperature).mean()
...
def build_class_weights(train_df, num_classes, device, exponent=0.15):
...
def build_hard_pair_matrix(num_classes, hard_negative_map_path, device):
    mat = torch.zeros((num_classes, num_classes), dtype=torch.float32)

    hard_map = load_json(hard_negative_map_path)

    for k, values in hard_map.items():
        try:
            src = int(k)
        except Exception:
            continue
        ...
        for v in values:
            ...
            if 0 <= src < num_classes and 0 <= dst < num_classes and src != dst:
                mat[src, dst] = 1.0
                mat[dst, src] = 1.0

    print("Hard pair matrix pairs:", int(mat.sum().item() / 2))
...
def build_hard_weight_by_label(train_df, hard_weight_max=1.8):
    tmp = (
...
def train_one_epoch(
    ...
    for batch in tqdm(loader, desc="M24 Metric Training", leave=False):
        ...
        with torch.amp.autocast("cuda", enabled=str(device).startswith("cuda")):
            ...
            hard_loss = hardpair_criterion(emb, labels)

            loss = (
                ce_weight * ce_loss
                + supcon_weight * supcon_loss
                + hardpair_weight * hard_loss
            )
        ...
        running_hard += hard_loss.item() * bs
...
def validate_one_epoch(model, loader, ce_criterion, device, num_classes):
    ...
    for batch in tqdm(loader, desc="M24 Metric Validation", leave=False):
        images = batch["image"].to(device)
        labels = batch["label"].to(device)
        hard_weights = batch["hard_weight"].to(device)
...
def main(args):
    ...
    train_dataset = M24HardMetricDataset(
        train_df,
        transform=train_tfms,
        hard_weight_max=args.hard_weight_max,
    )

    val_dataset = M24HardMetricDataset(
        val_df,
        transform=val_tfms,
        hard_weight_max=args.hard_weight_max,
    )
    ...
    print("Created M24MetricVisualModel")
    print("Backbone:", args.backbone_name)
    print("Embedding dim:", args.embedding_dim)
    print("Dropout:", args.dropout_p)
    print("CosFace scale:", args.cosface_scale)
    print("CosFace margin:", args.cosface_margin)
    ...
    hard_pair_matrix = build_hard_pair_matrix(
        num_classes=num_classes,
        hard_negative_map_path=args.hard_negative_map,
        device=device,
    )
    ...
    hardpair_criterion = HardPairNegativeLoss(
        hard_pair_matrix=hard_pair_matrix,
        temperature=args.hardpair_temperature,
        margin=args.hardpair_margin,
    )
    ...
    config = {
        ...
        "hardpair_weight": args.hardpair_weight,
        "cosface_scale": args.cosface_scale,
        "cosface_margin": args.cosface_margin,
        "supcon_temperature": args.supcon_temperature,
        "hardpair_temperature": args.hardpair_temperature,
        "hardpair_margin": args.hardpair_margin,
        "lr": args.lr,
        "weight_decay": args.weight_decay,
        "batch_effective_size": args.classes_per_batch * args.samples_per_class,
    }

    save_json(config, output_dir / "m24_metric_visual_config.json")
    save_json({str(k): int(v) for k, v in idx_to_label.items()}, output_dir / "idx_to_label.json")

    for epoch in range(1, args.epochs + 1):
        print(f"\nM24 Metric Visual Epoch [{epoch}/{args.epochs}]")
        ...
        train_result = train_one_epoch(
            ...
            hardpair_criterion=hardpair_criterion,
            optimizer=optimizer,
            scaler=scaler,
            device=device,
            ce_weight=args.ce_weight,
        ...
        print(
            ...
            f"HardPair: {train_result['hardpair_loss']:.4f} | "
        ...
        row = {
            ...
            "train_hardpair_loss": train_result["hardpair_loss"],
        ...
        checkpoint = {
            ...
            "val_macro_f1_all": val_result["macro_f1_all"],
            "val_acc": val_result["accuracy"],
            "model_type": "M24MetricVisualModel",
            "stage": "M24_metric_visual",
            "config": config,
        }
    ...
    print("\nM24 metric visual training done.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="M24 metric visual model with SupCon and hard-pair negative learning."
    )
    ...
    parser.add_argument("--hard_negative_map", type=str, required=True)
    parser.add_argument("--idx_to_label", type=str, required=True)

    parser.add_argument(
        "--output_dir",
        type=str,
        default="/content/drive/MyDrive/model/M24_metric_learning/train_run_v1",
    )

    parser.add_argument("--best_name", type=str, default="M24_metric_visual_best.pth")
    parser.add_argument("--last_name", type=str, default="M24_metric_visual_last.pth")
    ...
    parser.add_argument("--class_weight_exponent", type=float, default=0.15)
    parser.add_argument("--hard_weight_max", type=float, default=1.8)
    parser.add_argument("--label_smoothing", type=float, default=0.03)

    parser.add_argument("--cosface_scale", type=float, default=30.0)
    parser.add_argument("--cosface_margin", type=float, default=0.15)

    parser.add_argument("--ce_weight", type=float, default=1.0)
    parser.add_argument("--supcon_weight", type=float, default=0.10)
    parser.add_argument("--hardpair_weight", type=float, default=0.05)

    parser.add_argument("--supcon_temperature", type=float, default=0.07)
    parser.add_argument("--hardpair_temperature", type=float, default=0.10)
    parser.add_argument("--hardpair_margin", type=float, default=0.10)
