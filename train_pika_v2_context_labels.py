...
CSV_PATH = "/content/processed_pika_v2/pika_context_metadata.csv"
IMAGE_SIZE = 224
BATCH_SIZE = 32
EPOCHS = 10
LR = 3e-4
...
class PIKAV2Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        context_labels = json.loads(row["context_labels"])
        context_vector = np.zeros(self.num_classes, dtype=np.float32)

        for old_label in context_labels:
            mapped = row["label_map"].get(str(old_label), None)
            if mapped is not None:
                context_vector[int(mapped)] = 1.0

        context_vector = torch.tensor(context_vector, dtype=torch.float32)

        return pill_img, context_vector, label
...
class PIKAV2Model(nn.Module):
    def __init__(self, num_classes, pill_model_name):
        super().__init__()
    ...
    def forward(self, pill_img, context_vector):
        pill_feat = self.pill_encoder(pill_img)
        context_feat = self.context_encoder(context_vector)

        gate_input = torch.cat([pill_feat, context_feat], dim=1)
        gates = self.gate(gate_input)

        pill_gate = gates[:, 0].unsqueeze(1)
        context_gate = gates[:, 1].unsqueeze(1)

        gated_pill_feat = pill_feat * pill_gate
        gated_context_feat = context_feat * context_gate

        fused = torch.cat([gated_pill_feat, gated_context_feat], dim=1)
...
def train_one_epoch(model, loader, criterion, optimizer, device):
    ...
    for pill_imgs, context_vectors, labels in pbar:
        pill_imgs = pill_imgs.to(device)
        context_vectors = context_vectors.to(device)
        labels = labels.to(device)

        optimizer.zero_grad()
        outputs = model(pill_imgs, context_vectors)
...
def main():
    ...
    label_map_str = {str(k): int(v) for k, v in old_to_new.items()}
    df["label_map"] = [label_map_str] * len(df)
    ...
    train_dataset = PIKAV2Dataset(train_df, num_classes=num_classes, transform=train_tfms)
    val_dataset = PIKAV2Dataset(val_df, num_classes=num_classes, transform=val_tfms)
    ...
    for epoch in range(EPOCHS):
        ...
        if val_f1 > best_f1:
            ...
            torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, "best_pika_v2_model.pth"))
