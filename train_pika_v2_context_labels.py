...
class ContextPIKADataset(Dataset):
...
class ContextPIKAModel(nn.Module):
    def __init__(
        self,
        num_classes: int,
        pill_model_name: str,
    ...
    def forward(self, pill_img, pres_img, context_indices, context_mask):
        ...
        context_emb = self.context_embedding(safe_indices)
        ...
        context_feat = (attn_weights.unsqueeze(-1) * context_emb).sum(dim=1)

        gate_input = torch.cat([pill_feat, pres_feat, context_feat], dim=1)
        gates = self.gate(gate_input)

        pill_feat = pill_feat * gates[:, 0].unsqueeze(1)
        pres_feat = pres_feat * gates[:, 1].unsqueeze(1)
        context_feat = context_feat * gates[:, 2].unsqueeze(1)
        ...
        fused = torch.cat([
            ...
            context_feat,
            pill_feat * pres_feat,
            pill_feat * context_feat,
            pres_feat * context_feat,
        ], dim=1)
...
def main(args):
    ...
    train_dataset = ContextPIKADataset(
        train_df,
    ...
    val_dataset = ContextPIKADataset(
        val_df,
        max_context_len=max_context_len,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train M3 PIKA context-label model using predefined train/val split CSV")

    parser.add_argument("--train_csv", type=str, required=True)
    parser.add_argument("--val_csv", type=str, required=True)
    parser.add_argument("--test_csv", type=str, default="")

    parser.add_argument("--output_dir", type=str, default="/content/drive/MyDrive/model/M3_split_pika_context")
    parser.add_argument("--best_name", type=str, default="M3_pika_context_split_best.pth")
    parser.add_argument("--last_name", type=str, default="M3_pika_context_split_last.pth")
