...
CSV_PATH = "/content/processed_pika_v3/pika_v3_metadata.csv"
IMAGE_SIZE = 224
BATCH_SIZE = 20
EPOCHS = 10
LR = 3e-4
...
class PIKAV3Dataset(Dataset):
    def __init__(self, df, num_classes, transform=None, pres_transform=None):
...
class TripleContextPIKA(nn.Module):
    def __init__(self, num_classes, pill_model_name, pres_model_name):
        ...
        self.pill_proj = nn.Linear(pill_dim, 256)
        self.pres_proj = nn.Linear(pres_dim, 256)
        self.ctx_proj = nn.Linear(128, 256)
    ...
    def forward(self, pill_img, pres_img, context_vector):
        ...
        pres_gate = gates[:, 1].unsqueeze(1)
        ctx_gate = gates[:, 2].unsqueeze(1)

        pill_feat = pill_feat * pill_gate
        pres_feat = pres_feat * pres_gate
        ctx_feat = ctx_feat * ctx_gate
...
def main():
    ...
    train_dataset = PIKAV3Dataset(
        ...
        transform=train_tfms, pres_transform=train_tfms
    )
    val_dataset = PIKAV3Dataset(
        val_df, num_classes=num_classes,
        transform=val_tfms, pres_transform=val_tfms
    )
    ...
    for epoch in range(EPOCHS):
        ...
        if val_f1 > best_f1:
            ...
            torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, "best_pika_v3_model.pth"))
